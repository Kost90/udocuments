...
class UserService {
    save(user) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                validationHelper_1.default.checkForNullorUndefined(user, `${this.constructor.name}: user`);
                const savedUser = yield userRepository.save(user);
                // eslint-disable-next-line @typescript-eslint/no-unused-vars
                const { email, password } = savedUser, newUser = __rest(savedUser, ["email", "password"]);
                return newUser;
            }
            catch (error) {
