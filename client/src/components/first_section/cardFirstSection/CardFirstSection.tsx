...
function CardFirstSection({ lang }: { lang: UDocumentData }) {
  return (
    ...
    >
      ...
      <Paragraph
        text={lang.firstSection.paragraph}
        className="mt-3 md:mt-0 lg:text-lg"
      />
      <ul className="flex flex-col mt-4 lg:mt-0 ml-4 mr-4 mb-4">
        {lang.firstSection.servicesList.map((el, i) => (
          <li
            key={`${el}+${i}`}
            className="text-base-mobile text-cream text-wrap lg:text-base list-disc"
          >
            {el}
          </li>
        ))}
